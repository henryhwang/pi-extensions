...
export default function (pi: ExtensionAPI) {
	// Session tracking
	const stats = { total: 0, modes: {} as Record<string,number>, agents: new Map<string,number>(), last: null as string | null };
	const updateStat = (ctx: ExtensionContext) => ctx.ui.setStatus("subagent", stats.total > 0 ? `🚀 ${stats.total}` : "");
	const recordStat = (mode: string, agent: string, ctx: ExtensionContext) => { stats.total++; stats.modes[mode]=(stats.modes[mode]||0)+1; stats.agents.set(agent,(stats.agents.get(agent)||0)+1); stats.last=agent; updateStat(ctx); };
	pi.registerTool({
		name: "subagent",
		label: "Subagent",
		description: [
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.chain && params.chain.length > 0) {
				recordStat("chain", params.chain[0].agent, ctx);
			...
			if (params.agent && params.task) {
				recordStat("single", params.agent, ctx);
				const result = await runSingleAgent(
					ctx.cwd,
					agents,
					params.agent,
	...
	pi.on("session_start", (_e, c) => updateStat(c));

	// Contribute bundled workflow prompts as slash commands
