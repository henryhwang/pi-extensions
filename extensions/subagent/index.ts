...
export default function (pi: ExtensionAPI) {
  ...
  const updateStat = (ctx: ExtensionContext) =>
    ctx.ui.setStatus("subagent", stats.total > 0 ? `🚀 ${stats.total}` : "");
  const recordStat = (mode: string, agent: string, ctx: ExtensionContext) => {
    stats.total++;
  ...
  pi.registerCommand("subagent-stats", {
    description: "Show subagent stats",
    handler: async (_, ctx) => {
